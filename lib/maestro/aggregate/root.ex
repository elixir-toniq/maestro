defmodule Maestro.Aggregate.Root do
  ...
  * `:shutdown_policy` - controls idle process behavior as a keyword list with
  `:mode` and `:after` keys. `:after` is milliseconds of inactivity before the
  policy fires. Defaults to `[mode: :hibernate, after: 15_000]`). `:mode` is one
  of the following:
    - `:hibernate` — hibernate the process to reclaim memory
    - `:snapshot` — snapshot state and exit (requires `restart: :temporary`)
    - `:exit` — exit without snapshotting (requires `restart: :temporary`)
  """
  ...
  defmacro __using__(opts) do
    quote location: :keep do
      ...
      @shutdown_policy Keyword.get(@opts, :shutdown_policy,
                         mode: :hibernate,
                         after: 15_000
                       )
      @shutdown_after Keyword.get(@shutdown_policy, :after)
      ...
      def init(%Timestamp{} = id) do
        ...
        {:ok, agg, {:continue, :hydrate}}
      end

      def handle_call(:fetch, _from, agg) do
        agg = Root.update_aggregate(agg)
        {:reply, {:ok, agg.state}, agg, @shutdown_after}
      rescue
        err ->
          {:reply, {:error, err}, agg, @shutdown_after}
      end
      ...
      def handle_call({:replay, seq}, _from, agg) do
        {:reply, {:ok, Root.replay(agg, seq)}, agg, @shutdown_after}
      rescue
        err -> {:reply, {:error, err}, agg, @shutdown_after}
      end
      ...
      def handle_call(:get_snapshot, _from, agg) do
        snap =
          agg.state
          |> prepare_snapshot()
          |> Root.to_snapshot(agg.id, agg.sequence)

        {:reply, {:ok, snap}, agg, @shutdown_after}
      rescue
        err -> {:reply, {:error, err}, agg, @shutdown_after}
      end
      ...
      def handle_call({:eval_command, command, opts}, _from, agg) do
        case Root.eval_command(agg, command) do
          {:ok, agg, events} ->
            ...
            {:reply, result, agg, @shutdown_after}

          {:error, _reason} = error ->
            {:reply, error, agg, @shutdown_after}
        end
      rescue
        err -> {:reply, {:error, err}, agg, @shutdown_after}
      end

      def handle_continue(:hydrate, agg) do
        {:noreply, Root.update_aggregate(agg), @shutdown_after}
      end

      def handle_info(:timeout, %Root{sequence: 0} = agg) do
        {:stop, :normal, agg}
      end

      def handle_info(:timeout, agg) do
        case Keyword.get(@shutdown_policy, :mode) do
          :hibernate ->
            {:noreply, agg, :hibernate}

          :snapshot ->
            agg.state
            |> prepare_snapshot()
            |> Root.to_snapshot(agg.id, agg.sequence)
            |> Root.persist_snapshot()
  ...
  defmacro __before_compile__(_) do
    quote do
      ...
      def handle_info(_msg, agg), do: {:noreply, agg, @shutdown_after}
    end
  end
  ...
  @doc """
  Computes the event type string of the form "The.ModuleName -> the.module_name"
  dropping the provided prefix for conciseness
  """
  ...
  @doc false
  def to_snapshot(body, aggregate_id, curr_seq) do
    %Snapshot{
