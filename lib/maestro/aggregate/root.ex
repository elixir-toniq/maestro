defmodule Maestro.Aggregate.Root do
  ...
  * `:projectors` - zero or more modules that implement the `ProjectionHandler`
  behaviour for the events that are generated by this aggregate root
  """
  ...
  def eval_command(agg, command) do
    with seq <- Map.get(command, :sequence, 0),
         ...
         projections <- prepare_strong_projections(agg, events) do
      persist_events(agg, command, events, projections)
    end
  end
  ...
  defp persist_events(agg, command, events, projections) do
    case Store.commit_all(events, projections) do
      :ok ->
        apply_events(agg, events)
  ...
  @spec prepare_strong_projections(t(), [Maestro.Types.Event.t()]) :: [
          {any(), module(), function(), [any()]}
        ]
  defp prepare_strong_projections(agg, events) do
    agg.projectors
    |> Stream.filter(fn proj -> proj.strong?() end)
    |> Stream.flat_map(fn proj ->
      Enum.map(events, &proj.project(&1))
    end)
    |> Enum.filter(&some?/1)
  end

  defp some?(x), do: not is_nil(x)
